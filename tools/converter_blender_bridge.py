...
This bridge emits vanilla-safe model approximations so the result is visible
...
def render_view(path: Path, objects: list[bpy.types.Object], view: str) -> None:
    ...
    if view == "back":
        location = Vector((center.x, maximum.y + distance, center.z))
        ...
        location = Vector((minimum.x - distance, center.y, center.z))
        ortho_scale = max(extent.y, extent.z, 0.1) * 1.16
    elif view == "top":
        location = Vector((center.x, center.y, maximum.z + distance))
        ortho_scale = max(extent.x, extent.y, 0.1) * 1.16
    elif view == "bottom":
        location = Vector((center.x, center.y, minimum.z - distance))
        ortho_scale = max(extent.x, extent.y, 0.1) * 1.16
    else:
    ...
    camera_data.clip_end = distance * 8.0
...
def write_rendered_box_resourcepack(resourcepack_dir: Path, namespace: str, model_id: str, objects: list[bpy.types.Object]) -> int:
    ...
    for view in ("front", "back", "left", "right", "top", "bottom"):
        render_view(texture_dir / f"{model_id}_{view}.png", objects, view)

    thin = 0.04
    elements = [
        model_plane([0, 0, 0], [16, 16, thin], {"north": "#front", "south": "#front"}),
        model_plane([0, 0, 16 - thin], [16, 16, 16], {"north": "#back", "south": "#back"}),
        model_plane([0, 0, 0], [thin, 16, 16], {"west": "#left", "east": "#left"}),
        model_plane([16 - thin, 0, 0], [16, 16, 16], {"west": "#right", "east": "#right"}),
        model_plane([0, 16 - thin, 0], [16, 16, 16], {"up": "#top", "down": "#top"}),
        model_plane([0, 0, 0], [16, thin, 16], {"up": "#bottom", "down": "#bottom"}),
    ]

    model_json = {
        "credit": "Generated by RavoxModels Blender rendered-box converter",
        "textures": {
            "front": f"{namespace}:item/{model_id}_front",
            "back": f"{namespace}:item/{model_id}_back",
            "left": f"{namespace}:item/{model_id}_left",
            "right": f"{namespace}:item/{model_id}_right",
            "top": f"{namespace}:item/{model_id}_top",
            "bottom": f"{namespace}:item/{model_id}_bottom",
        },
        "elements": elements,
        "display": {
            "fixed": {"rotation": [0, 180, 0], "translation": [0, 0, 0], "scale": [2.4, 2.4, 2.4]},
...
def write_rendered_cross_resourcepack(resourcepack_dir: Path, namespace: str, model_id: str, objects: list[bpy.types.Object]) -> int:
    ...
    render_view(texture_dir / f"{model_id}_side.png", objects, "right")
...
def write_rendered_box_manifest(path: Path, elements: int, animations: list[dict[str, Any]]) -> None:
    manifest = {
        "renderer": "rendered_box_planes",
        "element_count": elements,
        "animations": animations,
        "notes": [
            "The source model is rendered into six transparent orthographic textures.",
            "Minecraft displays those textures on a thin 3D box. This is more stable than crossed planes and keeps high visual detail.",
            "It is still not true GLB triangle geometry; true mesh rendering requires a client mod.",
        ],
    }
    path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
...
def main() -> int:
    ...
    if args.model_mode == "rendered_box":
        element_count = write_rendered_box_resourcepack(resourcepack_dir, namespace, model_id, objects)
        write_rendered_box_manifest(out.parent / "minecraft-model-report.json", element_count, animations)
        return 0
